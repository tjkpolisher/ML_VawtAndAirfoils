function yplot3D20221212

clear all;
close all;
tn = 22;
ycuts = linspace(0.03, 0.12, 46);

%ycut = 0.06;
%z=;

xmin = 0.03; xmax = 0.12;
zmin = 0.01; zmax = 0.065;
Nz = 47; % 20 or 30 (1st injector only) / 52 (2nd injector included, calculated based on 30 when 1st injector only)
save_path = 'D:\newparticleStorage\numberOfParticles\20221212\nz47';
...
for ii=1:length(ycuts)
    ...
    for i=1:tn
        ...
        title(sprintf('$y$=%.3f, $t$=%d',ycut,i),'interpreter','latex')
        f = gcf;
        figurename = sprintf('numberOfParticles(transient)_y_%0.3f_time_%d_Nz_%d.jpg', ycut,i,Nz);
        figureName = char([save_path,'\',figurename]);
        exportgraphics(f,figureName,"Resolution",300,"BackgroundColor","none")
        %caxis([0 25])
