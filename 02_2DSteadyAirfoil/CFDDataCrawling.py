...
for i in folders:
    ...
    if not path.exists(copy_folderAirfoilName + "\\" + str(i).lower() + "coordinates.txt"):
        shutil.copy2(origin_coord + "\\" + str(i).lower() + "coordinates.txt",
                     copy_folderAirfoilName + "\\airfoil" + str(counter) + "coordinates.txt")
    ...
    for j in range(0, 16):
        ...
        if j==0:
            ...
            os.chdir(origin_data+"\\"+str(i)+"\\"+str(i)+"_files\\dp0\\FFF-"+str(j)+"\\Fluent")
        
        ## List of .out files in the current folder
        file_list = os.listdir()
        file_out_list = [file for file in file_list if file.endswith('.out')]
        
        Cd_file_name = copy_folderAirfoilName+"\\"+str(alpha) +"\\Cd_"+str(counter)+"_"+str(alpha)+".txt"
        Cl_file_name = copy_folderAirfoilName+"\\"+str(alpha) +"\\Cl_"+str(counter)+"_"+str(alpha)+".txt"
