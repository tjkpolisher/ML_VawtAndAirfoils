...
with mirrored_strategy.scope():
    ...
    x_ = tf.keras.layers.MaxPooling2D((2, 2), padding='same')(x_conv)
    reshape1 = tf.keras.layers.Reshape((12*n_units,))(x_)
    ...
    sd.save('AeroCNN_I_256kernels.h5', overwrite=True, include_optimizer=True, save_format='h5')

    test_results = sd.evaluate([x_train,aa_train])
    decoded_train = sd.predict([x_train,aa_train])
    decoded_test = sd.predict([x_test, aa_test])
